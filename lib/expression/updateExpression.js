...
class Updatable {
    ...
    serializeAdd(context) {
        ...
        const expression = (0, objectUtils_1.mapToArray)(this.addMap)
            .map(([path, value]) => {
    ...
    serializeDelete(context) {
        ...
        const expression = (0, objectUtils_1.mapToArray)(this.deleteMap)
            .map(([path, subset]) => {
            const pathExpression = path.serialize(context);
    ...
    serializeRemove(context) {
        ...
        const expression = (0, objectUtils_1.setToArray)(this.removeSet)
    ...
    serializeSet(context) {
        ...
        const expression = (0, objectUtils_1.mapToArray)(this.setMap)
            .map(([path, value]) => {
            const pathExpression = path.serialize(context);
            const valueExpression = value.serialize(context);
            return `${pathExpression} = ${valueExpression}`;
